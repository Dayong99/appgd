		...
		<view class="boxsrocll" v-if="current==1">
			<view class="searchvalu">
				...
				<view class="serachinput">
					...
					<input type="text" placeholder="通过预制场或预制台座编号" @click="searcgvaky">
				</view>
			</view>
		</view>
		<view v-if="current==0">
	...
	onShow(){
		...
		beamfield(data).then(violaRes=>{
			...
			this.$store.commit('album/beamfieldList', violaRes.data.result.records)
			   // this.beamfieldList = violaRes.data.result.records
			// console.log(violaRes.data.result.records.length)
