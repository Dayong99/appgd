		      		...
		      		<view class="beamList" style="">
		      			...
		      			<view class="plancontent">
		      				<view v-for="child in item.deviceInspectionPlanSubList" class="planchildlist">
		      					 <view class="planterm">
		      						<text class="goplan" :class="{'colortype':child.inspectionStatus==1}">{{child.inspectionStatus_dictText}}</text>
		      						<text class="godetails">{{child.inspectionContent}}</text>
		      					</view>
		      					<view class="contengbox">
		      						<view class="botmeg">
		      							<image class="ren" src="../../static/dingwei2.png"></image>
		      							<text style="display: inline-block;margin-right: 35rpx;">{{child.inspectors_dictText}}</text>
		      							<image class="locations" src="../../static/dingwei1.png"></image>
		      							<text>{{child.deviceId_dictText}}</text>
		      						</view>
		      						 <view class="routing" @click="searcgvaky('examine',child)" v-if="child.inspectionStatus===0">去检查</view>
		      					</view> 
		      				</view>
		      			</view>
		      		</view>
					...
					<view class="beamList" style="">
						...
						<view class="plancontent">
							<view v-for="child in item.deviceMaintainPlanSubList" class="planchildlist">
								 <view class="planterm">
									<text class="goplan":class="{'colortype':child.maintainStatus==1}">{{child.maintainStatus_dictText}}</text>
									<text class="godetails">{{child.maintainContent}}</text>
								</view>
								<view class="contengbox">
									<view class="botmeg">
										<image class="ren" src="../../static/dingwei2.png"></image>
										<text style="display: inline-block;margin-right: 35rpx;">{{child.maintainers_dictText}}</text>
										<image class="locations" src="../../static/dingwei1.png"></image>
										<text>{{child.deviceId_dictText}}</text>
									</view>
				...
				<view v-for="(item,index) in getservice" :key="index" class="beamListbox" >
					<view class="beamList" style="">
						<view class="nameplan" style="width: 615rpx;margin-left: -8rpx;">
							<view class="startend">
								<text style="display:inline-block;">
									申报时间:{{item.operatorTime | planFilt(item.operatorTime)}}<text style="display:inline-block;margin:0 8rpx;">至</text>{{item.createTime | planFilt(item.operatorTime)}}
								</text>
							</view>
						</view>
						<view class="plancontent">
							<view class="planchildlist">
								 <view class="planter-mdeclare">
									<text class="goplan" :class="{'colortype':item.operatorType==0}">{{item.operatorType_dictText}}</text>
									<text class="godetails">{{item.describes}}</text>
								</view>
								<view class="contengbox">
									<view class="botmeg">
										<image class="ren" src="../../static/dingwei2.png"></image>
										<text style="display: inline-block;margin-right: 35rpx;">{{item.operator_dictText}}</text>
										<image class="locations" src="../../static/dingwei1.png"></image>
										<text>{{item.deviceId_dictText}}</text>
									</view>
									 <view class="routing" @click="searcgvaky('equipment',item)">去维护</view>
								</view> 
							</view>
						</view>
					</view>
    ...
    methods: {
	  ...
	  upCallback(mescroll) {
	    [inspecttask,upkeeptask,servicetask][this.current]({
		  ...
		  userPhone:this.$store.getters['user/getAccountInfoData'].username,
...
<style>
	...
	.plancontent .planterm .colortype{
