  ...
  <view class="" style="position: relative;top:0;bottom: 0;background: #fff;z-index: 999;">
    <!-- 滚动内容区 -->
		<view class="head flex mauto cwhite sw f34r">
			...
			<view class="safe fcenter">设备保养</view>
			<view>
		...
		<view>
			<view class="processlist">
				<view class="listleft"><text class="start">*</text>保养时间</view>
				<view class="listrig" @click="onShowDatePicker('datetime')">
					<view style="color: #E3E3E3;" v-if="datepent==1">请选择保养时间</view>
			...
			<view class="processlist">
				<view class="listleft"><text class="start">*</text>保养人</view>
				<view class="listrig">
					<picker @change="changePeople" :value="personIndex" :range="personList" :range-key="'personName'">
						...
						<view class="uni-input" style="color: #E3E3E3;" v-else>请选择保养人</view>
					</picker>
				</view>
				<image src="../../static/jkpe.png" class="poarrow"></image>
			</view>
			<view class="processlist" style="height: 213rpx;">
				<view class="listleft"><text class="start">*</text>保养描述</view>
				<view class="listrig textlist">
					<textarea placeholder="请输入保养描述" v-model="updatedetails">
...
import {  personList,deviceMaintainPlanMain } from "@/api/AlbumServer.js";
export default {
  data() {
    return {
      datepent: 1,
  ...
  methods: {
    ...
    yesword() {
      ...
      let data = {
		...
		maintainContent: this.updatedetails,
        filePath: this.updateimg.toString(),
        relTenantIds: this.$store.getters["user/getAccountInfoData"]
          .relTenantIds,
        operatorTime: this.datetime.replace(/\//g, "-") + ":00",
        deviceId: this.parentVal.id,
        userPhone: this.personList[this.personIndex].phone,
      
      };
      console.log(data);
      deviceMaintainPlanMain(data).then((res) => {
        console.log(res);
        if (res.status == 200) {
          uni.showToast({
            title: "保养成功",
