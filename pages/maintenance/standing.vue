<template>
  <view class="" style="position: relative;top:0;bottom: 0;z-index: 999;height:100%;">
		<view class="head flex mauto cwhite sw f34r">
			...
			<view class="safe fcenter">设备台账</view>
			<view class="check" @click="searcgvaky('gorectifyorder')">
			
			</view>
		</view>
		<view class="boxsrocll">
			<scroll-view class="scroll-bar-find" :scroll-x="true" :show-scrollbar="false" :scroll-into-view="scrollInto" style=" border-right:1rpx solid #fff;"
			...
			('Morescreening')">搜索</view>
		</view> 
		<view>
			<mescroll-uni @down="downCallback" @up="upCallback" :top="242" @init="mescrollInit">
    ...
    methods: {
	  ...
	  upCallback(mescroll) {
	    devicestand({
		  ...
		  relTenantIds: this.$store.getters[
