					...
					<view v-for="(item,index) in dangeimg" :key="index" class="beamListbox" v-if="current==1">
						<view class="beamList" style="padding-bottom:35rpx">
							<view class="nameplan">
								...
								<view class="plan" style="" v-if="item.orderStatus==0||item.orderStatus==1"
									@click="releent(item,index)">{{orderList1[item.orderStatus]}}</view>
								<view class="accept" style="" v-if="item.orderStatus==2">
									<image :src="'../../static/'+(item.checkStatus==0?'dagou':'gantanhao')+'.png'">
									</image>
									<text v-if="item.checkStatus==0" style="color:#2EA62E">验收合格</text>
									<text v-if="item.checkStatus==1">验收不合格</text>
								</view>
							</view>
							<view class="hiddendanger">
								...
								<view class="inspection">巡检人:{{item.inspector_dictText}}</view>
								<view class="inspection">整改期限:{{item.inspectValidityTime}}</view>
	...
	export default {
		...
		methods: {
			...
			hiddendetails(val) {
				hiddendanger({
					planId: val.id,
					orderType: 1, //1-安全巡检-巡检计划
					relTenantIds: this.$store.getters['user/getAccountInfoData'].relTenantIds,
				}).then(res => {
					console.log('参数', val.id)
					console.log('点击隐患整改详情res', res)
					if (res.status == 200 && res.data.result.records.length) {
						let data = res.data.result.records[0]
						// 隐患整改
						if (data.repairPath) {
							console.log('隐患整改详情repairPath', data.repairPath)
							data.repairPath = data.repairPath.split(',')
						}
						// 巡检计划
						if (data.inspectPath) {
							console.log('安全巡检-巡检计划-隐患整改详情inspectPath', data.inspectPath)
							data.inspectPath = data.inspectPath.split(',')
						}
						uni.navigateTo({
							url:`../gorectifydetail/gorectifydetail?valent=${JSON.stringify(data)}&pollingType=1`
						})
					} else {
						uni.showToast({
							title: '没有查询到隐患整改详情',
			...
			searcgvaky(val, item) {
				if (item) {
					...
					if (val == 'gorectifydetail') {
						console.log('详情', item)
						if (item.repairPath && typeof(item.repairPath)=='string') {
			...
			fntabClick1(tab, index) {
				...
				if (this.indexList1.includes(0) && this.indexList1.includes(1) && this.indexList1.includes(2)) {
					this.searchIdString = '0,1,2'
				} else if (this.indexList1.includes(0) && this.indexList1.includes(1) && !this.indexList1.includes(2)) {
					this.searchIdString = '0,1'
				} else if (this.indexList1.includes(0) && !this.indexList1.includes(1) && this.indexList1.includes(2)) {
					this.searchIdString = '0,2'
				} else if (!this.indexList1.includes(0) && this.indexList1.includes(1) && this.indexList1.includes(2)) {
					this.searchIdString = '1,2'
				} else if (!this.indexList1.includes(0) && !this.indexList1.includes(1) && this.indexList1.includes(2)) {
					this.searchIdString = '2'
				} else if (this.indexList1.includes(0) && !this.indexList1.includes(1) && !this.indexList1.includes(2)) {
					this.searchIdString = '0'
				} else if (!this.indexList1.includes(0) && this.indexList1.includes(1) && !this.indexList1.includes(2)) {
					this.searchIdString = '1'
				} else if (!this.indexList1.includes(0) && !this.indexList1.includes(1) && !this.indexList1.includes(2)) {
					this.searchIdString = ''
				}
			...
			upCallback(mescroll) {
				[safepolling, hiddendanger][this.current]({
					...
					if (this.current == 1) {
						...
						console.log('安全巡检-隐患整改筛选结果',violaRes.data.result.records)
						if (mescroll.num == 1) {
