<template>
	<view class="" style="position: relative;top:0;bottom: 0;z-index: 999;height:100%;">
		<view class="head flex mauto cwhite sw f34r">
			...
			<view class="safe fcenter">质量巡检</view>
		...
		methods: {
			releent(val) {
				if (val.orderStatus == 0) {
					uni.navigateTo({
						url: '../gorectify/gorectify?valent=' + JSON.stringify(val) + '&pollingType=0'
					})
				} else if (val.orderStatus == 1) {
					uni.navigateTo({
						url: '../gocheck/gocheck?valent=' + JSON.stringify(val) + '&pollingType=0'
					})
			...
			hiddendetails(val) {
				hiddendanger({
					...
					if (res.status == 200 && res.data.result.records.length) {
						...
						if (data.inspectPath && typeof(data.inspectPath) == 'string') {
			...
			searcgvaky(val, item) {
				if (item) {
					...
					uni.navigateTo({
						url: `../${val}/${val}?valent=${items}&pollingType=0`
					})
				} else {
					uni.navigateTo({
						url: `../${val}/${val}?pollingType=0`
					})
				}
			},
			/// 跳转打开新窗口
