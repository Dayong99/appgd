<template>
  <view>
	 ...
	 <view id="tabbar" class="boxlistd mauto flexr-jsa flex-ais hl90r bgwhite bbdash2r tabbar-fixed nice arrowtoplr">
	 		<view class="f34r fbold fcenter w128r tabbar" :class="{'tabbarsh':current == 0}" @tap="fnBarClick(0)">全部</view>
	 		<view class="f34r fbold fcenter w128r tabbar" :class="{'tabbarsh':current == 1}" @tap="fnBarClick(1)">收入</view>
	 		<view class="f34r fbold fcenter w128r tabbar" :class="{'tabbarsh':current == 2}" @tap="fnBarClick(2)">支出</view>
	 </view> 
	 
    <!-- 滚动列表内容 -->
	<mescroll-uni :down="{use:false}" :top="516" @up="upCallback" @init="mescrollInit">
	<!-- 选择导航 -->
	  <!-- <view v-if="isFixed" class="hl90r"></view> -->
	  
	  <!-- 全部 -->
	  <view v-if="status.hottest" :style="{display: current==0 ? 'block' :'none'}" class="boxlistd mauto bgwhite arrowbottomlr">
	  ...
	  <!-- 收入 -->
	  <view v-if="status.latest" :style="{display: current==1 ? 'block' :'none'}" class="boxlistd mauto bgwhite arrowbottomlr">
	  ...
	  <!-- 支出 -->
	  <view v-if="status.hottest" :style="{display: current==2 ? 'block' :'none'}" class="boxlistd mauto bgwhite arrowbottomlr">
        ...
        getTopicReplyList(params).then(res => {
			if (this.current ==0) {
				if (mescroll.num == 1) {
				  ...
				  this.$store.commit('topic/setTopicHottestListData', this.topicHottestListData.concat(res.data.result.records))
				}
			}
			if (this.current == 1) {
			  if (mescroll.num == 1) {
			    ...
			    this.$store.commit('topic/setTopicLatestListData', this.topicLatestListData.concat(res.data.result.records))
			  }
			}
			if (this.current == 2) {
			  if (mescroll.num == 1) {
			    ...
			    this.$store.commit('topic/setTopicReplyInfoData', this.topicLatestList.concat(res.data.result.records))
			  }
			}	
