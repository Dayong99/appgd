		...
		<view class="wagdetail" style="padding-top:153rpx">
			...
			<view class="flex listwag">
				...
				<view class="uni-list-cell-db">
					<picker @change="bindPickerChange4" :value="index4" :range="array4">
						<view class="uni-input">{{array4[index4]?array4[index4]:''}}</view>
					</picker>
				</view>
				<view class="arrowicon">
					<image src="../../static/jkpe.png" class="hw100v"></image>
			...
			<view class="flex listwag">
				...
				<picker mode="multiSelector" :range="dateTimeArray" v-model="dateTime" @change="change"
					...
					<view>
						<input type="text" disabled="true" v-model="upTower" style="text-indent: 20rpx;"
							placeholder="请输入起止时间" placeholder-class="inputPlace" />
					</view>
			...
			<view class="flex listwag">
				...
				<view></view>
				<picker mode="multiSelector" :range="dateTimeArray1" v-model="dateTime1" @change="change1"
					@columnchange="columnchange1">
					<view>
						<input type="text" disabled="true" v-model="upTower1" style="text-indent: 20rpx;"
							placeholder="请输入终止时间" placeholder-class="inputPlace" />
					</view>
					<image src="../../static/dates.png" class="wagongicon"></image>
				</picker>
			</view>
	...
	export default {
		...
		methods: {
			...
			change1(e) {
				...
				// })
				e.detail.value.forEach((val, index) => {
					value.push(this.dateTimeArray1[index][val])
				})
				let dateArray = value[0] + "-" + value[1] + "-" + value[2] + " " + value[3] + ":" + value[4]
