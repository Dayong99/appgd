<template>
	<view>
		...
		<view id="tabbar" class="boxlistd mauto flexr-jsa flex-ais hl90r bgwhite bbdash2r tabbar-fixed wagontitle">
			<view class="f30r svb fcenter w128r tabbar sw" :class="{'tabbarsh':current == 0}" @tap="fnBarClick(0)">收料
			</view>
			<view class="f30r svb fcenter w128r tabbar sw" :class="{'tabbarsh':current == 1}" @tap="fnBarClick(1)">发料
			</view>
			<view class="f30r svb fcenter w128r tabbar sw" :class="{'tabbarsh':current == 2}" @tap="fnBarClick(2)">直进直出
			</view>
		</view>

		<!-- 滚动列表内容 -->
		<mescroll-uni :down="{use:false}" :top="160" @up="upCallback" @init="mescrollInit">
			<!-- 选择导航 -->
				...
				getWordContentHTML(params).then(res => {
					...
					if (this.current == 0) {
						if (mescroll.num == 1) {
							...
							this.$store.commit('word/setWordListData', this.topicHottestListData.concat(res.data
								.result.records))
							console.log('收料', this.topicHottestListData)
						}
					}
					// 发料
					if (this.current == 1) {
						if (mescroll.num == 1) {
							...
							this.$store.commit('word/setWordInfoDataList', this.topicLatestList.concat(res.data
								.result.records))
							console.log('发料', this.topicLatestList)
						}
					}
					// 直进直出
					if (this.current == 2) {
						if (mescroll.num == 1) {
							...
							this.$store.commit('word/setWordInfoData', this.topicLatestListData.concat(res.data
								.result.records))
							console.log('直进直出', this.topicLatestListData)
						}
					}
